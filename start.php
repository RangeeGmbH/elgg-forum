...
function forum_init() {
    ...
    elgg_register_widget_type('forumtopic', elgg_echo('forumtopic:widget:title'), elgg_echo('forumtopic:widget:description'));
...
function forum_entity_menu_setup($hook, $type, $return, $params) {
    ...
    if (elgg_instanceof($entity, "object", "forum")) {
        $url = $entity->getURL();
        $item = new ElggMenuItem("topic_count", elgg_echo("forum:count:topics", array(forum_count_topics($entity))), $url);
        array_unshift($return, $item);
        $item = new ElggMenuItem("reply_count", elgg_echo("forum:count:replies", array(forum_count_replies($entity))), $url);
        array_unshift($return, $item);
    }
    if (elgg_instanceof($entity, "object", "forumtopic")) {
        $url = $entity->getURL();
        $item = new ElggMenuItem("reply_count", elgg_echo("forumtopic:count:replies", array(topic_count_replies($entity))), $url);
        array_unshift($return, $item);
        $item = new ElggMenuItem("topic_view", elgg_echo("forumtopic:count:views", array(topic_count_views($entity))), $url);
        array_unshift($return, $item);
    }
    if (elgg_instanceof($entity, "object", "forumreply")) {
        ...
        case "forumcategory":
            switch ($vars[0]) {
                ...
                case "view":
                    ...
                    elgg_push_breadcrumb(elgg_get_excerpt($category->getDisplayName()), "forumcategory/view/" . $vars[1]);
                    break;
                case "edit":
                    $guid = $vars[1];
                    $category = get_entity($guid);
